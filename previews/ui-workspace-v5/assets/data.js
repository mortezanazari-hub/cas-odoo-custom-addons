window.CAS_DATA = (() => {
  ...
  const roleCapabilities = {
    employee:["workspace.home","action.read","request.create","request.track","report.daily.create","attendance.self","correspondence.self"],
    supervisor:["team.dashboard","team.actions","team.reports","attendance.team.review","shift.team.read","approval.decide"],
    manager:["management.dashboard","team.actions","team.reports","attendance.team.review","shift.manage","approval.decide"],
    ceo:["executive.dashboard","approval.decide","organization.analytics"],
    guard:["guard.workspace","guard.event.create","guard.event.read","shift.self","report.daily.create"],
    secretariat:["secretariat.workspace","correspondence.manage","document.manage"],
    shift_planner:["shift.manage","attendance.import"],
    form_designer:["form.design"],
    workflow_designer:["workflow.design"],
    document_manager:["document.manage","ocr.manage"],
    system_admin:["admin.center","admin.users","admin.roles","admin.organization","admin.positions","admin.assignments","admin.settings","admin.audit","admin.health","technical.tools"]
  };

  const navGroups = [
    {title:"فضای کاری",items:[
      {route:"home",label:"خانه",icon:"⌂",permissions:["workspace.home"]},
      {route:"my-actions",label:"کارهای من",icon:"✓",permissions:["action.read"],count:12},
