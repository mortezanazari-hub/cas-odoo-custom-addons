(() => {
  ...
  function renderHome(){
    ...
    return `<div class="page employee-workspace-v5">
      <section class="briefing-hero">
        <div class="briefing-copy">
          ...
          <div class="briefing-lines">
            <button data-route="my-actions"><span class="briefing-bullet urgent">!</span><strong>۳ مورد نیازمند اقدام شماست</strong><small>یک مورد در آستانه عبور از SLA</small></button>
            <button data-route="calendar"><span class="briefing-bullet">◷</span><strong>اولین جلسه ساعت ۱۱:۰۰ است</strong><small>جلسه هماهنگی عملیات</small></button>
            <button data-route="work-report-new"><span class="briefing-bullet warning">▤</span><strong>گزارش دیروز هنوز نهایی نشده</strong><small>حدود ۳۰ دقیقه فعالیت ثبت‌نشده باقی مانده</small></button>
          </div>
        </div>
        <div class="briefing-side">
          <div class="workspace-presence"><span class="presence-dot"></span><div><strong>حضور فعال از ۰۷:۵۸</strong><small>شیفت صبح • ۰۷:۳۰ تا ۱۹:۳۰</small></div><button class="btn btn-sm" data-route="attendance-hub">جزئیات</button></div>
          <button class="command-launcher" data-route="global-search-page"><span>⌕</span><div><strong>چه کاری می‌خواهی انجام بدهی؟</strong><small>فرم، اقدام، شخص، سند یا فرایند را جست‌وجو کن</small></div><kbd>Ctrl K</kbd></button>
        </div>
      </section>
      ...
      <section class="action-strip">
        ...
        <button data-route="calendar"><span>◷</span><div><strong>برنامه‌ریزی</strong><small>افزودن رویداد یا جلسه</small></div></button>
      ...
      <div class="command-work-grid">
        <section class="card personal-todo-card">
          ...
          <div class="todo-tabs"><button class="active">امروز <b>۳</b></button><button>آینده <b>۱</b></button><button>عقب‌افتاده <b>۱</b></button><button>انجام‌شده</button></div>
          <div class="card-body"><div class="personal-task-list">${renderPersonalTasks()}</div></div>
          <div class="card-footer"><button class="btn btn-ghost" data-route="personal-tasks">نمایش همه کارها</button></div>
        </section>

        <section class="card calendar-compact-card">
          <div class="card-header"><div><div class="card-eyebrow">زمان‌بندی امروز</div><h3 class="card-title">تقویم</h3><div class="card-subtitle">شنبه ۲۷ تیر</div></div><div class="calendar-view-switch"><button class="active">روز</button><button>هفته</button><button>ماه</button></div></div>
          <div class="card-body">
            <div class="calendar-timeline">
              <div class="calendar-now-line"><span>اکنون ۰۹:۴۵</span></div>
              <button class="calendar-event event-blue" style="--start:8;--span:1.2"><time>۱۱:۰۰</time><div><strong>جلسه هماهنگی عملیات</strong><small>اتاق جلسات مدیریت • ۶۰ دقیقه</small></div></button>
              <button class="calendar-event event-green" style="--start:12;--span:1"><time>۱۴:۳۰</time><div><strong>مرور گزارش‌های شیفت</strong><small>گزارش کار • ۴۵ دقیقه</small></div></button>
              <button class="calendar-event event-purple" style="--start:16;--span:.8"><time>۱۶:۰۰</time><div><strong>پاسخ به مکاتبات باز</strong><small>دبیرخانه • ۳۰ دقیقه</small></div></button>
            </div>
          </div>
          <div class="card-footer"><button class="btn" data-route="calendar">بازکردن تقویم کامل</button><button class="btn btn-primary" data-action="new-calendar-event">+ رویداد جدید</button></div>
        </section>
      </div>

      <div class="workspace-middle-grid">
        <section class="card action-center-card"><div class="card-header"><div><div class="card-eyebrow">آنچه از دیگر بخش‌ها به شما رسیده</div><h3 class="card-title">نیازمند اقدام</h3><div class="card-subtitle">درخواست، تأیید، Workflow و پیگیری‌ها در یک نمای عملیاتی</div></div><button class="btn btn-sm" data-route="my-actions">مشاهده همه</button></div><div class="card-body">${actionRows(3)}</div></section>

        <section class="card day-progress-compact">
          <div class="card-header"><div><div class="card-eyebrow">آنچه واقعاً انجام شده</div><h3 class="card-title">گزارش امروز</h3></div><strong>${toFaMinutes(totalMinutes)}</strong></div>
          <div class="card-body"><div class="progress-track"><span style="width:${Math.min(100,Math.round(totalMinutes/480*100))}%"></span></div><div class="compact-progress-stats"><div><span>ثبت‌شده</span><strong>${num(workActivities.length)} فعالیت</strong></div><div><span>پوشش روز کاری</span><strong>${num(Math.min(100,Math.round(totalMinutes/480*100)))}٪</strong></div></div></div>
          <div class="card-footer"><button class="btn" data-action="add-activity-focus">ثبت فعالیت</button><button class="btn btn-primary" data-route="work-report-new">مرور گزارش</button></div>
        </section>
      </div>

      <section class="card quick-activity-card v5-quick-activity">
        <div class="card-header"><div><div class="card-eyebrow">ثبت سریع فعالیت انجام‌شده</div><h3 class="card-title">الان روی چه کاری بودی؟</h3><div class="card-subtitle">این بخش گزارش کار است، نه فهرست کارهای آینده.</div></div><span class="kbd-hint">Alt + A</span></div>
        <div class="card-body">
          <div class="quick-activity-form">
            <div class="field quick-activity-select"><label>فعالیت</label><select id="quickActivity"><option value="بررسی و پاسخ به مکاتبات عملیات|مکاتبات">بررسی و پاسخ به مکاتبات عملیات</option><option value="تحلیل مغایرت حضور شیفت شب|حضور و کاردکس">تحلیل مغایرت حضور شیفت شب</option><option value="پیگیری اقدام‌های باز|اقدام‌ها">پیگیری اقدام‌های باز</option><option value="جلسه هماهنگی عملیات|جلسه">جلسه هماهنگی عملیات</option><option value="__new__">فعالیت موردنظر در فهرست نیست...</option></select></div>
            <div class="field quick-duration"><label>مدت</label><select id="quickDuration"><option value="15">۱۵ دقیقه</option><option value="30" selected>۳۰ دقیقه</option><option value="45">۴۵ دقیقه</option><option value="60">۱ ساعت</option><option value="90">۱ ساعت و ۳۰ دقیقه</option></select></div>
            <div class="field quick-note"><label>نتیجه یا توضیح <span>اختیاری</span></label><input id="quickNote" placeholder="خروجی یا نکته مهم..." /></div>
            <button class="btn btn-primary quick-add-btn" data-action="add-quick-activity">+ ثبت فعالیت</button>
          </div>
        </div>
      </section>

      <section class="card today-activity-card collapsed-activity-card">
        <div class="card-header"><div><h3 class="card-title">آخرین فعالیت‌های ثبت‌شده</h3><div class="card-subtitle">گزارش روزانه به‌تدریج از این موارد ساخته می‌شود.</div></div><div class="activity-header-total"><span>مجموع</span><strong>${toFaMinutes(totalMinutes)}</strong></div></div>
        <div class="card-body"><div class="activity-list">${renderTodayActivities()||`<div class="empty-state"><div class="empty-icon">＋</div><h3>هنوز فعالیتی ثبت نشده</h3></div>`}</div></div>
      </section>

      <div class="workspace-bottom-grid v5-bottom">
        <section class="card personal-shortcuts"><div class="card-header"><div><h3 class="card-title">میانبرهای من</h3><div class="card-subtitle">مسیرهای پرتکرار، نه ساختار ماژول‌های پشت صحنه</div></div><button class="mini-btn" data-action="customize-workspace">⚙</button></div><div class="card-body"><div class="shortcut-grid"><button data-route="request-center"><span>◫</span><strong>درخواست‌های من</strong><small>ثبت و پیگیری</small></button><button data-route="messages"><span>✉</span><strong>پیام‌ها</strong><small>گفت‌وگو و کانال‌ها</small></button><button data-route="attendance-hub"><span>◴</span><strong>حضور و شیفت</strong><small>کاردکس شخصی</small></button><button data-route="my-documents"><span>▧</span><strong>اسناد مرتبط</strong><small>فایل‌ها و نسخه‌ها</small></button></div></div></section>
        <section class="card system-notices"><div class="card-header"><div><h3 class="card-title">فقط برای اطلاع</h3><div class="card-subtitle">این موارد نیازمند اقدام فوری نیستند</div></div></div><div class="card-body"><div class="notice-line"><span>!</span><div><strong>کاردکس خرداد قفل شد</strong><small>اصلاح فقط از مسیر بازگشایی مجاز است.</small></div></div><div class="notice-line"><span>ش</span><div><strong>فیلترهای تاریخ شمسی به‌روزرسانی شد</strong><small>در جست‌وجوهای پیشرفته قابل استفاده است.</small></div></div></div></section>
      </div>
    </div>`;
  }
  ...
  function renderPersonalTasksPage(){
    const filtered=personalTasks.map((t,i)=>({...t,_index:i})).filter(taskFilter);
      ...
      <div class="v6-task-layout">
        <aside class="card v6-filter-panel">
          ...
          <div class="card-body v6-filter-list">
            ...
            <hr><span class="filter-caption">دسته‌ها</span>
            ${["شخصی","پیگیری","تفویض‌شده","جلسه","اداری"].map(x=>`<button><span>● ${x}</span></button>`).join("")}
          </div>
        </aside>
  ...
  function chatMessages(name){
    ...
    return samples.map(m=>`<div class="chat-message ${m[3]}"><div class="avatar avatar-sm">${initials(m[0])}</div><div><div class="message-meta"><strong>${m[0]}</strong><time>${m[2]}</time></div><p>${m[1]}</p>${m[2]==="۰۹:۲۱"?`<button class="chat-attachment"><span>▧</span><div><strong>صورتجلسه-عملیات.pdf</strong><small>۱.۲ MB • PDF</small></div></button>`:""}</div></div>`).join("");
  }

  function renderMessagesPage(){
    const c=conversations[selectedConversation];
    return `<div class="page v6-page">${pageHeader("پیام‌ها","گفت‌وگوی مستقیم، کانال واحدی و کانال سازمانی بر بستر زیرساخت Mail.","ارتباطات سازمانی",`<button class="btn" data-action="message-search">جست‌وجو</button><button class="btn btn-primary" data-action="new-conversation">+ گفت‌وگوی جدید</button>`)}
      <section class="v6-messenger">
        <aside class="conversation-panel">
          <div class="conversation-search">⌕<input placeholder="جست‌وجوی گفتگو و پیام..." /></div>
          <div class="conversation-tabs"><button class="active">همه</button><button>خوانده‌نشده</button><button>کانال‌ها</button><button>مستقیم</button></div>
          <div class="conversation-list">${conversations.map((x,i)=>`<button class="conversation-item ${i===selectedConversation?"active":""}" data-action="select-conversation" data-index="${i}"><span class="avatar">${x.avatar}</span><div><strong>${x.name}</strong><p>${x.preview}</p></div><aside><time>${x.time}</time>${x.unread?`<b>${num(x.unread)}</b>`:""}</aside></button>`).join("")}</div>
        </aside>
        <main class="chat-panel">
          <header class="chat-header"><div class="avatar">${c.avatar}</div><div><strong>${c.name}</strong><small>${c.kind} • ${c.members}</small></div><div class="chat-header-actions"><button>⌕</button><button data-action="chat-files">فایل‌ها</button><button data-action="chat-info">اطلاعات</button></div></header>
          <div class="chat-context"><span>مرتبط با:</span><button>درخواست خرید FRM/۱۴۰۵/۰۰۳۶۶</button><button>نامه ۱۴۰۵/د/۰۰۲۴</button></div>
          <div class="chat-body"><div class="chat-date">امروز، ۲۷ تیر</div>${chatMessages(c.name)}</div>
          <footer class="chat-compose"><div class="compose-tools"><button>＋</button><button>☺</button><button>▧</button></div><textarea id="chatInput" placeholder="پیام خود را بنویسید..."></textarea><button class="send-button" data-action="send-chat-message">ارسال</button></footer>
        </main>
        <aside class="chat-info-panel"><div class="avatar avatar-lg">${c.avatar}</div><h3>${c.name}</h3><p>${c.kind}</p><div class="chat-info-actions"><button>بی‌صدا</button><button>سنجاق</button><button>اعضا</button></div><hr><h4>فایل‌های مشترک</h4><div class="shared-file"><span>PDF</span><div><strong>صورتجلسه عملیات</strong><small>امروز</small></div></div><div class="shared-file"><span>XLS</span><div><strong>گزارش شیفت شب</strong><small>دیروز</small></div></div></aside>
      </section>
    </div>`;
  }
  ...
  function renderRecentHistory(){
    const history=[
      ...
      ["۰۹:۳۱","ارسال پیام به مریم فلاحی","پیام‌ها","✉"],
  ...
  function addPersonalTaskModal(){
    openModal("افزودن کار شخصی",`<div class="proposal-note"><strong>ساده و سریع</strong><p>برای ساخت یک کار، فقط عنوان کافی است. زمان، دسته و جزئیات اختیاری‌اند.</p></div><div class="form-grid"><div class="field full"><label class="required">عنوان کار</label><input id="personalTaskTitle" placeholder="مثلاً پیگیری قرارداد سرویس دستگاه" /></div><div class="field"><label>زمان انجام</label><select id="personalTaskTime"><option>امروز</option><option>فردا</option><option>این هفته</option><option>بدون تاریخ</option></select></div><div class="field"><label>دسته‌بندی</label><select id="personalTaskSource"><option>شخصی</option><option>پیگیری</option><option>اداری</option><option>جلسه</option></select></div><div class="field full"><label>توضیح</label><textarea placeholder="اختیاری..."></textarea></div></div>`,`<button class="btn" data-action="close-layer">انصراف</button><button class="btn btn-primary" data-action="confirm-personal-task">افزودن کار</button>`);
  }

  function newCalendarEventModal(){
    openModal("رویداد جدید",`<div class="form-grid"><div class="field full"><label class="required">عنوان رویداد</label><input placeholder="عنوان جلسه یا برنامه..." /></div><div class="field"><label class="required">تاریخ</label><input value="۱۴۰۵/۰۴/۲۷" /></div><div class="field"><label class="required">زمان شروع</label><input type="time" value="15:00" /></div><div class="field"><label>زمان پایان</label><input type="time" value="16:00" /></div><div class="field"><label>نوع</label><select><option>جلسه</option><option>کار زمان‌بندی‌شده</option><option>یادآوری</option><option>رویداد سازمانی</option></select></div><div class="field full"><label>شرکت‌کنندگان یا توضیح</label><textarea placeholder="اختیاری..."></textarea></div></div>`,`<button class="btn" data-action="close-layer">انصراف</button><button class="btn btn-primary" data-action="demo-save">ثبت رویداد</button>`);
  }
  ...
  function handleAction(action,target){
    switch(action){
      ...
      case "send-chat-message": {
        const input=document.getElementById("chatInput"); if(!input?.value.trim()){toast("متن پیام خالی است.","warn");break;} input.value="";toast("پیام ارسال شد."); break;
      }
      ...
      case "chat-info": toast("اطلاعات و اعضای گفتگو نمایش داده شد."); break;
