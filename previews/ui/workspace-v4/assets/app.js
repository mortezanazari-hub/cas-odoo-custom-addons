(() => {
  ...
  function renderHome(){
    ...
    return `<div class="page employee-workspace-v4">
      <div class="workspace-welcome">
        <div><div class="page-kicker">شنبه ۲۷ تیر ۱۴۰۵ • فضای شخصی</div><h1>صبح بخیر، ${esc(firstName)}</h1><p>کارهای مهم امروز را ببین، فعالیت‌ها را همان لحظه ثبت کن و گزارش روزانه‌ات را تدریجی کامل کن.</p></div>
      ...
      <div class="workspace-focus-grid">
        <section class="card quick-activity-card">
          <div class="card-header"><div><div class="card-eyebrow">ثبت سریع فعالیت</div><h2 class="quick-title">الان روی چه کاری بودی؟</h2><div class="card-subtitle">فعالیت را در چند ثانیه به گزارش امروز اضافه کن.</div></div><span class="kbd-hint">Alt + A</span></div>
          <div class="card-body">
            <div class="quick-activity-form">
              ...
              <div class="field quick-duration"><label>مدت</label><select id="quickDuration"><option value="15">۱۵ دقیقه</option><option value="30" selected>۳۰ دقیقه</option><option value="45">۴۵ دقیقه</option><option value="60">۱ ساعت</option><option value="90">۱ ساعت و ۳۰ دقیقه</option><option value="120">۲ ساعت</option></select></div>
              <div class="field quick-note"><label>توضیح کوتاه <span>اختیاری</span></label><input id="quickNote" placeholder="نتیجه یا نکته مهم فعالیت..." /></div>
              <button class="btn btn-primary quick-add-btn" data-action="add-quick-activity">+ افزودن به امروز</button>
            </div>
            <div class="quick-suggestions"><span>پرتکرار:</span><button data-action="quick-pick" data-value="پیگیری اقدام‌های باز|اقدام‌ها">پیگیری اقدام‌ها</button><button data-action="quick-pick" data-value="جلسه هماهنگی عملیات|جلسه">جلسه</button><button data-action="quick-pick" data-value="بررسی و پاسخ به مکاتبات عملیات|مکاتبات">مکاتبات</button></div>
          </div>
        </section>

        <section class="card day-progress-card">
          <div class="card-header"><div><div class="card-eyebrow">گزارش امروز</div><h3 class="card-title">پیشرفت ثبت فعالیت‌ها</h3></div><strong class="day-total">${toFaMinutes(totalMinutes)}</strong></div>
          <div class="card-body"><div class="progress-ring" style="--progress:${Math.min(100,Math.round(totalMinutes/480*100))}%"><div><strong>${num(Math.min(100,Math.round(totalMinutes/480*100)))}٪</strong><span>از روز کاری</span></div></div><div class="day-summary"><div><span>فعالیت ثبت‌شده</span><strong>${num(workActivities.length)}</strong></div><div><span>زمان باقی‌مانده پیشنهادی</span><strong>${toFaMinutes(Math.max(0,480-totalMinutes))}</strong></div></div></div>
          <div class="card-footer"><button class="btn" data-route="work-report-new">مرور گزارش کامل</button><button class="btn btn-primary" data-action="submit-work-report">ثبت نهایی روز</button></div>
        </section>
      </div>

      <div class="workspace-main-grid">
        <section class="card actions-focus-card"><div class="card-header"><div><h3 class="card-title">کارهای نیازمند اقدام</h3><div class="card-subtitle">اولویت‌بندی‌شده بر اساس فوریت و مهلت انجام</div></div><button class="btn btn-sm" data-route="my-actions">مشاهده همه</button></div><div class="card-body">${actionRows(4)}</div></section>
        <section class="card today-plan-card"><div class="card-header"><div><h3 class="card-title">برنامه امروز</h3><div class="card-subtitle">قرارها و نقاط زمانی مهم</div></div><button class="mini-btn" title="افزودن برنامه">+</button></div><div class="card-body"><div class="compact-agenda"><div class="agenda-now"><time>اکنون</time><div><strong>پیگیری اقدام‌های باز</strong><small>تا ۱۰:۳۰ • فضای شخصی</small></div></div><div><time>۱۱:۰۰</time><div><strong>جلسه هماهنگی عملیات</strong><small>اتاق جلسات مدیریت</small></div></div><div><time>۱۴:۳۰</time><div><strong>مرور گزارش‌های شیفت</strong><small>گزارش کار</small></div></div><div><time>۱۶:۰۰</time><div><strong>پاسخ به مکاتبات باز</strong><small>دبیرخانه</small></div></div></div></div></section>
      </div>

      <section class="card today-activity-card"><div class="card-header"><div><h3 class="card-title">فعالیت‌های امروز</h3><div class="card-subtitle">گزارش روزانه به‌صورت تدریجی از همین موارد ساخته می‌شود.</div></div><div class="activity-header-total"><span>مجموع</span><strong>${toFaMinutes(totalMinutes)}</strong></div></div><div class="card-body"><div class="activity-list">${renderTodayActivities()||`<div class="empty-state"><div class="empty-icon">＋</div><h3>هنوز فعالیتی ثبت نشده</h3><p>از فرم ثبت سریع بالا استفاده کنید.</p></div>`}</div></div></section>

      <div class="workspace-bottom-grid">
        <section class="card personal-shortcuts"><div class="card-header"><div><h3 class="card-title">میانبرهای من</h3><div class="card-subtitle">دسترسی‌های پرتکرار قابل شخصی‌سازی</div></div><button class="mini-btn" data-action="customize-workspace">⚙</button></div><div class="card-body"><div class="shortcut-grid"><button data-route="request-center"><span>◫</span><strong>درخواست جدید</strong><small>مرخصی، مأموریت و...</small></button><button data-route="correspondence"><span>✉</span><strong>مکاتبات من</strong><small>نامه‌ها و ارجاع‌ها</small></button><button data-route="attendance-hub"><span>◴</span><strong>حضور و شیفت</strong><small>کاردکس شخصی</small></button><button data-route="my-documents"><span>▧</span><strong>اسناد من</strong><small>فایل‌های مرتبط</small></button></div></div></section>
        <section class="card system-notices"><div class="card-header"><div><h3 class="card-title">اطلاعیه‌های مرتبط</h3><div class="card-subtitle">فقط موارد مؤثر بر کار شما</div></div></div><div class="card-body"><div class="notice-line"><span>!</span><div><strong>کاردکس خرداد قفل شد</strong><small>اصلاح فقط از مسیر بازگشایی مجاز است.</small></div></div><div class="notice-line"><span>ش</span><div><strong>فیلترهای تاریخ شمسی به‌روزرسانی شد</strong><small>در جست‌وجوهای پیشرفته قابل استفاده است.</small></div></div></div></section>
      </div>
    </div>`;
  }
