(() => {
  ...
  const v7Defaults = {
    ...
    widgetOrder: ["tasks","calendar","actions","conversations","activity","notices"],
    hiddenWidgets: [],
    shortcuts: ["new-request","add-personal-task","new-calendar-event","add-activity-focus","correspondence-new","messages","attendance-hub","my-documents"],
    shortcutSuggestions: true,
    widgetSettings: {
      calendar: {defaultView:"day",showMeetings:true,showShifts:true,showLeaves:true,showHolidays:false},
      tasks: {maxItems:"4",showCompleted:false,sort:"priority"},
      actions: {maxItems:"3",onlyUrgent:false},
      conversations: {maxItems:"4",onlyUnread:false},
      activity: {compact:true,defaultDuration:"30",showRecent:true},
      "day-progress": {targetMinutes:"480"},
      notices: {maxItems:"4",onlyImportant:false}
    }
  };
  let v7Settings = Object.assign({}, v7Defaults, JSON.parse(localStorage.getItem("cas.v7.settings") || "{}"));
  v7Settings.widgetSettings=Object.assign({},v7Defaults.widgetSettings,v7Settings.widgetSettings||{});
  Object.keys(v7Defaults.widgetSettings).forEach(k=>v7Settings.widgetSettings[k]=Object.assign({},v7Defaults.widgetSettings[k],v7Settings.widgetSettings[k]||{}));
  ...
  let homeCalendarView = localStorage.getItem("cas.home.calendarView") || v7Settings.widgetSettings.calendar.defaultView || "day";
  ...
  function sectionBack(route="attendance-hub",label="بازگشت به نمای حضور و شیفت"){
    return `<button class="section-back" data-route="${route}"><span>→</span>${label}</button>`;
  }
  ...
  function navItemAllowed(item){
    if(item.children) return item.children.some(child=>allowed(child.route));
    return allowed(item.route);
  }

  function parentNavFor(route){
    for(const group of D.navGroups){
      for(const item of group.items){
        if(item.children && item.children.some(child=>child.route===route)) return item;
      }
    }
    return null;
  }
  ...
  function renderSidebar(){
    const saved=JSON.parse(localStorage.getItem("cas.nav.open")||"{}");
    const html = D.navGroups.map(group => {
      const items = group.items.filter(navItemAllowed);
      if(!items.length) return "";
      return `<section class="nav-group"><div class="nav-group-title">${group.title}</div>${items.map(i=>{
        if(!i.children) return `<button class="nav-item ${i.route===currentRoute?"active":""}" data-route="${i.route}"><span class="nav-icon">${i.icon}</span><span class="nav-label">${i.label}</span>${i.count?`<span class="nav-count">${num(i.count)}</span>`:""}</button>`;
        const children=i.children.filter(child=>allowed(child.route));
        const active=children.some(child=>child.route===currentRoute);
        const open=active || saved[i.id]!==false;
        const count=children.reduce((sum,child)=>sum+(child.count||0),0);
        const parentRoute = i.route || children[0]?.route || "";
        return `<div class="nav-tree ${open?"open":""}" data-nav-tree="${i.id}"><div class="nav-parent-row"><button class="nav-item nav-parent ${active?"active":""}" ${parentRoute?`data-route="${parentRoute}"`:''}><span class="nav-icon">${i.icon}</span><span class="nav-label">${i.label}</span>${count?`<span class="nav-count">${num(count)}</span>`:""}</button><button class="nav-expander" data-nav-toggle="${i.id}" aria-label="${open?"بستن":"بازکردن"} زیرمنوی ${esc(i.label)}" aria-expanded="${open}" title="${open?"بستن":"بازکردن"} زیرمنو"><span aria-hidden="true">⌄</span></button></div><div class="nav-children" aria-hidden="${!open}"><div class="nav-children-inner">${children.map(child=>`<button class="nav-child ${child.route===currentRoute?"active":""}" data-route="${child.route}"><span class="nav-child-dot"></span><span>${child.label}</span>${child.count?`<b>${num(child.count)}</b>`:""}</button>`).join("")}</div></div></div>`;
      }).join("")}</section>`;
    }).join("");
  ...
  function setBreadcrumb(){
    ...
    const parent=parentNavFor(currentRoute);
    breadcrumb.innerHTML = parent && parent.route!==currentRoute
      ? `<button data-route="${parent.route||parent.children[0].route}">فضای کار</button><span>‹</span><button data-route="${parent.route||parent.children[0].route}">${esc(parent.label)}</button><span>‹</span><strong>${esc(meta.label)}</strong>`
      : `<button data-route="home">فضای کار</button><span>‹</span><strong>${esc(meta.label)}</strong>`;
    ...
    if(homeCalendarView==="day"){
      ...
      body=`<div class="home-month-shell">
        <div class="home-month-toolbar">
          <button class="home-month-nav" data-action="home-calendar-month-prev" aria-label="ماه قبل" title="ماه قبل">‹</button>
          <div><strong>${monthNames[monthIndex]} ${num(year)}</strong><small>نمای ماهانه تقویم شمسی</small></div>
          <button class="home-month-nav" data-action="home-calendar-month-next" aria-label="ماه بعد" title="ماه بعد">›</button>
        </div>
  ...
  function renderCompactActivityWidget(totalMinutes){
    const recent=workActivities.slice(0,4);
    const activityOptions=["بررسی و پاسخ به مکاتبات عملیات","تحلیل مغایرت حضور شیفت شب","پیگیری اقدام‌های باز","جلسه هماهنگی عملیات","فعالیت در فهرست نیست"];
    return `<section class="card dashboard-widget compact-activity-widget widget-full" data-widget="activity" draggable="true">
      ...
      <div class="card-body">
        <div class="compact-activity-form">
          <div class="searchable-activity-control activity-combobox" data-combobox="activity">
            <button type="button" class="activity-combobox-trigger" data-action="toggle-activity-combobox" aria-haspopup="listbox" aria-expanded="false"><span id="quickActivityLabel">انتخاب یا جست‌وجوی فعالیت</span><b>⌄</b></button>
            <input id="quickActivity" type="hidden" value="" />
            <div class="activity-combobox-panel" id="quickActivityPanel" hidden>
              <div class="activity-combobox-search"><span>⌕</span><input id="quickActivitySearch" autocomplete="off" placeholder="نام فعالیت را جست‌وجو کنید..." aria-label="جست‌وجوی فعالیت" /></div>
              <div class="activity-combobox-options" id="quickActivityResults" role="listbox">${activityOptions.map(x=>`<button type="button" role="option" data-action="select-quick-activity" data-value="${esc(x)}">${esc(x)}</button>`).join("")}</div>
            </div>
          </div>
          <div class="duration-control"><select id="quickDuration"><option value="15" ${v7Settings.widgetSettings.activity.defaultDuration==="15"?"selected":""}>۱۵ دقیقه</option><option value="30" ${v7Settings.widgetSettings.activity.defaultDuration==="30"?"selected":""}>۳۰ دقیقه</option><option value="45">۴۵ دقیقه</option><option value="60" ${v7Settings.widgetSettings.activity.defaultDuration==="60"?"selected":""}>۱ ساعت</option><option value="custom">زمان دلخواه</option></select><input id="quickDurationCustom" class="duration-custom-input" type="number" min="1" max="1440" step="1" placeholder="تعداد دقیقه" title="زمان دلخواه به دقیقه" hidden /></div>
        ...
        <div class="compact-activity-list fixed-row-list">${recent.map((item,i)=>`<div class="compact-activity-row ${item.standard===false?'proposed-activity-row':''}"><span class="activity-state">${item.standard===false?'◌':'✓'}</span><div><strong>${esc(item.title)} ${item.standard===false?'<em class="inline-proposal-badge">عنوان پیشنهادی</em>':''}</strong><small>${esc(item.category)} • ${toFaMinutes(item.duration)}${item.standard===false?' • در انتظار استانداردسازی':''}</small></div><button class="mini-btn" data-action="remove-work-activity" data-index="${i}">×</button></div>`).join("")}${emptyWidgetRows(Math.max(0,4-recent.length),"activity")}</div>
      </div>
  ...
  function applySavedWidgetOrder(){
    ...
    const hidden=new Set(Array.isArray(v7Settings.hiddenWidgets)?v7Settings.hiddenWidgets:[]);
    order.forEach(id=>{const item=grid.querySelector(`[data-widget="${id}"]`);if(item)grid.appendChild(item);});
    grid.querySelectorAll(".dashboard-widget").forEach(widget=>{widget.hidden=hidden.has(widget.dataset.widget);});
    const hero=document.querySelector('[data-dashboard-section="hero"]');
    if(hero) hero.hidden=hidden.has("hero");
    document.querySelector(".hidden-widgets-restore")?.remove();
    enableWidgetDrag();
  }
  ...
  const dashboardWidgetLabels={hero:"مرکز فرمان شخصی",tasks:"کارهای من",calendar:"تقویم",actions:"نیازمند اقدام",conversations:"گفت‌وگوهای اخیر",activity:"گزارش کار",notices:"اطلاع‌رسانی"};
  const dashboardShortcutRegistry={
    "new-request":{icon:"＋",title:"ثبت درخواست",subtitle:"مرخصی، مأموریت، خرید و...",action:"new-request"},
    "add-personal-task":{icon:"✓",title:"کار جدید",subtitle:"ثبت سریع در کارهای من",action:"add-personal-task"},
    "new-calendar-event":{icon:"◷",title:"رویداد جدید",subtitle:"جلسه یا برنامه شخصی",action:"new-calendar-event"},
    "add-activity-focus":{icon:"▤",title:"ثبت فعالیت",subtitle:"افزودن به گزارش امروز",action:"add-activity-focus"},
    "correspondence-new":{icon:"✉",title:"نامه جدید",subtitle:"مکاتبه رسمی سازمانی",route:"correspondence-new"},
    "messages":{icon:"◫",title:"گفت‌وگوها",subtitle:"ارتباط مستقیم و گروهی",route:"messages"},
    "attendance-hub":{icon:"◴",title:"حضور و شیفت",subtitle:"کاردکس شخصی",route:"attendance-hub"},
    "my-documents":{icon:"▧",title:"اسناد مرتبط",subtitle:"فایل‌ها و نسخه‌ها",route:"my-documents"},
    "personal-tasks":{icon:"☑",title:"همه کارها",subtitle:"مدیریت کامل کارهای شخصی",route:"personal-tasks"},
    "calendar":{icon:"▦",title:"تقویم کامل",subtitle:"روز، هفته و ماه",route:"calendar"}
  };
  function renderDashboardShortcuts(){
    const selected=Array.isArray(v7Settings.shortcuts)?v7Settings.shortcuts:v7Defaults.shortcuts;
    return selected.map(id=>{const x=dashboardShortcutRegistry[id]; if(!x)return ""; const dest=x.route?`data-route="${x.route}"`:`data-action="${x.action}"`; return `<button ${dest}><span>${x.icon}</span><div><strong>${x.title}</strong><small>${x.subtitle}</small></div></button>`;}).join("");
  }

  function dashboardSettingsDrawer(){
    const hidden=new Set(v7Settings.hiddenWidgets||[]);
    const shortcutIds=Object.keys(dashboardShortcutRegistry);
    const ws=v7Settings.widgetSettings;
    openDrawer("تنظیمات میزکار",`
      <div class="dashboard-settings-intro"><strong>میزکار شخصی شما</strong><p>نمایش ویجت‌ها، میانبرها و رفتار هر ویجت فقط برای حساب شما ذخیره می‌شود. روی میزکار هیچ نشانه‌ای از موارد مخفی نمایش داده نخواهد شد.</p></div>
      <div class="dashboard-settings-section"><h3>ویجت‌های قابل نمایش</h3><p>ویجت‌های موردنیاز را روشن یا خاموش کنید.</p><div class="dashboard-visibility-list">${Object.entries(dashboardWidgetLabels).map(([id,label])=>`<button class="dashboard-toggle-row ${hidden.has(id)?'off':'on'}" data-action="toggle-dashboard-widget" data-widget="${id}"><span><strong>${label}</strong><small>${hidden.has(id)?'مخفی در میزکار':'نمایش در میزکار'}</small></span><i>${hidden.has(id)?'خاموش':'روشن'}</i></button>`).join('')}</div></div>
      <div class="dashboard-settings-section"><h3>میانبرهای سریع</h3><p>میانبرها را اضافه یا حذف کنید و ترتیب نمایش آن‌ها را تغییر دهید.</p><div class="dashboard-shortcut-list">${shortcutIds.map(id=>{const x=dashboardShortcutRegistry[id],selected=(v7Settings.shortcuts||[]),on=selected.includes(id),idx=selected.indexOf(id);return `<div class="dashboard-shortcut-editor ${on?'on':'off'}"><button class="dashboard-toggle-row ${on?'on':'off'}" data-action="toggle-dashboard-shortcut" data-shortcut="${id}"><span><strong>${x.title}</strong><small>${x.subtitle}</small></span><i>${on?'حذف':'افزودن'}</i></button>${on?`<div class="shortcut-order-controls"><button class="btn btn-sm" data-action="move-dashboard-shortcut-up" data-shortcut="${id}" ${idx<=0?'disabled':''} title="انتقال به ابتدای فهرست">↑</button><button class="btn btn-sm" data-action="move-dashboard-shortcut-down" data-shortcut="${id}" ${idx>=selected.length-1?'disabled':''} title="انتقال به انتهای فهرست">↓</button></div>`:''}</div>`}).join('')}</div><label class="settings-check"><input type="checkbox" id="shortcutSuggestions" ${v7Settings.shortcutSuggestions?'checked':''}> نمایش پیشنهادهای خودکار سیستم در کنار میانبرهای دستی</label></div>
      <div class="dashboard-settings-section"><h3>تنظیمات اختصاصی ویجت‌ها</h3>
        <details open><summary>تقویم</summary><div class="widget-settings-grid"><label>نمای پیش‌فرض<select data-widget-setting="calendar.defaultView"><option value="day" ${ws.calendar.defaultView==='day'?'selected':''}>روز</option><option value="week" ${ws.calendar.defaultView==='week'?'selected':''}>هفته</option><option value="month" ${ws.calendar.defaultView==='month'?'selected':''}>ماه</option></select></label>${[["showMeetings","جلسات"],["showShifts","شیفت‌ها"],["showLeaves","مرخصی‌ها"],["showHolidays","تعطیلات"]].map(([k,l])=>`<label class="settings-check"><input type="checkbox" data-widget-setting="calendar.${k}" ${ws.calendar[k]?'checked':''}> نمایش ${l}</label>`).join('')}</div></details>
        <details><summary>کارهای من</summary><div class="widget-settings-grid"><label>حداکثر تعداد<select data-widget-setting="tasks.maxItems"><option ${ws.tasks.maxItems==='4'?'selected':''}>4</option><option ${ws.tasks.maxItems==='6'?'selected':''}>6</option><option ${ws.tasks.maxItems==='8'?'selected':''}>8</option></select></label><label>مرتب‌سازی<select data-widget-setting="tasks.sort"><option value="priority" ${ws.tasks.sort==='priority'?'selected':''}>اولویت</option><option value="due" ${ws.tasks.sort==='due'?'selected':''}>موعد</option></select></label><label class="settings-check"><input type="checkbox" data-widget-setting="tasks.showCompleted" ${ws.tasks.showCompleted?'checked':''}> نمایش انجام‌شده‌ها</label></div></details>
        <details><summary>گزارش کار</summary><div class="widget-settings-grid"><label>مدت پیش‌فرض<select data-widget-setting="activity.defaultDuration"><option value="15" ${ws.activity.defaultDuration==='15'?'selected':''}>۱۵ دقیقه</option><option value="30" ${ws.activity.defaultDuration==='30'?'selected':''}>۳۰ دقیقه</option><option value="60" ${ws.activity.defaultDuration==='60'?'selected':''}>۱ ساعت</option></select></label><label class="settings-check"><input type="checkbox" data-widget-setting="activity.compact" ${ws.activity.compact?'checked':''}> حالت فشرده</label><label class="settings-check"><input type="checkbox" data-widget-setting="activity.showRecent" ${ws.activity.showRecent?'checked':''}> نمایش فعالیت‌های اخیر</label></div></details>
        <details><summary>نیازمند اقدام و اعلان‌ها</summary><div class="widget-settings-grid"><label class="settings-check"><input type="checkbox" data-widget-setting="actions.onlyUrgent" ${ws.actions.onlyUrgent?'checked':''}> فقط موارد فوری</label><label class="settings-check"><input type="checkbox" data-widget-setting="notices.onlyImportant" ${ws.notices.onlyImportant?'checked':''}> فقط اعلان‌های مهم</label></div></details>
      </div>
    `,`<button class="btn" data-action="reset-dashboard-settings">بازنشانی</button><button class="btn btn-primary" data-action="save-dashboard-settings">ذخیره و اعمال</button>`);
  }
  ...
  function renderHome(){
    ...
    return `<div class="page employee-workspace-v5">
      <section class="briefing-hero briefing-hero-v2" data-dashboard-section="hero">
        <div class="briefing-header-row">
          ...
          <div class="dashboard-head-actions">
            <button class="presence-inline" data-route="attendance-hub" title="مشاهده حضور و شیفت">
              <span class="presence-dot"></span>
              <span><strong>حاضر از ۰۷:۵۸</strong><small>شیفت صبح تا ۱۹:۳۰</small></span>
        ...
        <button class="command-launcher command-launcher-wide" data-action="global-search">
          <span>⌕</span>
      ...
      <section class="action-strip unified-shortcuts">${renderDashboardShortcuts()}</section>
  ...
  function renderActions(urgent=false){
    const sourceGroups={
      attendance:a=>a.source==="حضور و غیاب",
      process:a=>["منابع انسانی","گزارش کار","فرم و فرایند"].includes(a.source),
      correspondence:a=>a.source==="مکاتبات"
    };
    let list = urgent ? D.actions.filter(a=>a.tone==="red"||a.tone==="orange") : [...D.actions];
    if(actionSourceFilter!=="all" && sourceGroups[actionSourceFilter]) list=list.filter(sourceGroups[actionSourceFilter]);
    if(actionTimeFilter==="today") list=list.filter(a=>a.deadline.includes("امروز"));
    if(actionTimeFilter==="near") list=list.filter(a=>a.tone==="red"||a.tone==="orange");
    const counts={
      all:D.actions.length,
      attendance:D.actions.filter(sourceGroups.attendance).length,
      process:D.actions.filter(sourceGroups.process).length,
      correspondence:D.actions.filter(sourceGroups.correspondence).length
    };
    const sourceChips=[
      ["all","همه منابع"],["attendance","حضور و غیاب"],["process","فرم و فرایند"],["correspondence","مکاتبات"]
    ].map(([value,label])=>`<button class="filter-chip ${actionSourceFilter===value?"active":""}" data-action="action-source-filter" data-value="${value}" aria-pressed="${actionSourceFilter===value}">${label}<b>${num(counts[value])}</b></button>`).join("");
    const timeChips=[["all","همه"],["today","امروز"],["near","نزدیک سررسید"]].map(([value,label])=>`<button class="${actionTimeFilter===value?"active":""}" data-action="action-time-filter" data-value="${value}">${label}</button>`).join("");
    return `<div class="page">${pageHeader(urgent?"اقدام‌های فوری":"کارتابل من",urgent?"مواردی که عبور از SLA یا اثر عملیاتی فوری دارند.":"اقدام‌های قابل انجام از همه ماژول‌ها؛ انجام کار روی رکورد منبع صورت می‌گیرد.","CAS Action Hub",`<button class="btn">همگام‌سازی</button><button class="btn btn-primary">اقدام گروهی</button>`)}
      ...
      <div class="actions-filter-panel" style="margin-top:18px">
        <div class="actions-filter-top"><div class="search-box">⌕<input placeholder="جست‌وجو در عنوان، منبع یا شخص مرتبط..." /></div><div class="segmented action-time-filter" aria-label="فیلتر زمانی">${timeChips}</div><select class="select action-sort"><option>مرتب‌سازی: فوریت</option><option>مهلت</option><option>اولویت</option></select></div>
        <div class="actions-source-row"><span>منبع اقدام</span><div class="action-source-chips" role="radiogroup" aria-label="منبع اقدام">${sourceChips}</div></div>
      </div>
      <section class="card"><div class="card-body">${list.length?`<div class="task-list">${list.map((a,i)=>`<div class="task-item"><div class="task-source">${a.icon}</div><div><h4>${a.title}</h4><p>${a.subject}</p><div class="task-meta">${badge(a.source,"blue")}${badge(a.status,a.tone)}</div></div><div class="task-deadline"><strong>${a.deadline}</strong><span>اولویت ${a.priority}</span><button class="btn btn-sm btn-ghost" data-action="open-action" data-index="${D.actions.indexOf(a)}">بررسی</button></div></div>`).join("")}</div>`:`<div class="empty-state"><div class="empty-icon">✓</div><h3>موردی با این فیلتر وجود ندارد</h3><p>یک منبع دیگر را انتخاب کنید یا فیلتر را به «همه منابع» برگردانید.</p></div>`}</div></section>
    </div>`;
  }
  ...
  function renderFormsAdmin(){
    return `<div class="page">${pageHeader("مدیریت فرم‌های سازمانی","تعریف پایدار، نسخه‌های پیش‌نویس/منتشرشده، مالک فرایند و وضعیت استفاده.","CAS Form Core",`<button class="btn" data-route="form-builder">طراح دیداری فرم</button><button class="btn">ورود از قالب</button><button class="btn btn-primary" data-action="new-form">تعریف فرم جدید</button>`)}
  ...
  function renderFormBuilder(){
    const palette = [["T","متن کوتاه"],["¶","متن بلند"],["#","عدد"],["٪","درصد"],["◉","انتخاب تکی"],["☑","چندانتخاب"],["ش","تاریخ جلالی"],["◴","ساعت"],["▱","فایل"],["▦","رکورد مرتبط"],["👤","کارمند"],["ƒ","محاسبه‌شده"],["▣","دسته‌بندی گزارش کار"],["↻","عنوان فعالیت تکرارشونده"],["▦","ماتریس پویا"]];
...
<div class="canvas-field source-backed-field"><span class="drag-handle">⠿</span><div class="form-grid"><div class="field"><label>دسته‌بندی کار</label><select><option>خواندن از منبع دسته‌بندی گزارش کار</option></select><small>Provider: Work Report Taxonomy</small></div><div class="field"><label>عنوان فعالیت تکرارشونده</label><select><option>براساس دسته‌بندی و نقش کاربر</option></select><small>Provider: Recurring Activity Catalog</small></div></div></div>
<div class="canvas-field matrix-builder-field selected"><span class="drag-handle">⠿</span><div class="field full"><label>ماتریس پایش روزانه</label><div class="builder-matrix"><table><thead><tr><th>پرسنل</th><th>حضور</th><th>گزارش کار</th><th>توضیح</th></tr></thead><tbody><tr><th>علی رضایی</th><td><input type="checkbox" checked></td><td><select><option>کامل</option></select></td><td><input placeholder="اختیاری"></td></tr><tr><th>مریم فلاحی</th><td><input type="checkbox"></td><td><select><option>ناقص</option></select></td><td><input placeholder="اختیاری"></td></tr></tbody></table></div><small>ردیف‌ها از منبع «پرسنل قابل مشاهده» و ستون‌ها از فیلدهای انتخاب‌شده تولید می‌شوند.</small></div></div></div></div><aside class="builder-panel"><div class="builder-title">ویژگی‌های بخش انتخاب‌شده</div><div class="property-list"><div class="field"><label>عنوان</label><input value="اطلاعات درخواست" /></div><div class="field"><label>کلید فنی</label><input value="request_info" dir="ltr" /></div><div class="field"><label>تعداد ستون</label><select><option>۲ ستون</option></select></div><h4>رفتار</h4><label style="font-size:10px;display:flex;gap:7px"><input type="checkbox" checked /> نمایش عنوان بخش</label><label style="font-size:10px;display:flex;gap:7px;margin-top:8px"><input type="checkbox" /> جمع‌شونده</label><h4>تنظیمات ماتریس</h4><div class="field"><label>منبع ردیف‌ها</label><select><option>پرسنل قابل مشاهده</option><option>زیرمجموعه مستقیم</option><option>واحد انتخاب‌شده</option></select></div><div class="field"><label>ستون‌ها</label><div class="chip-cloud"><span class="badge badge-blue">حضور</span><span class="badge badge-blue">گزارش کار</span><span class="badge badge-blue">توضیح</span></div></div><label style="font-size:10px;display:flex;gap:7px"><input type="checkbox" checked /> قفل ستون هویت پرسنل</label><label style="font-size:10px;display:flex;gap:7px;margin-top:8px"><input type="checkbox" checked /> صفحه‌بندی برای داده‌های زیاد</label><h4>اعتبارسنجی</h4><div class="field"><label>شرط نمایش</label><select><option>همیشه نمایش داده شود</option></select></div><button class="btn btn-danger btn-sm" style="width:100%;margin-top:10px">حذف بخش</button></div></aside></div>
    </div>`;
  }
  ...
  function renderShiftSwaps(){
    return `<div class="page">${sectionBack()}${pageHeader("جابه‌جایی شیفت","انتقال کنترل‌شده بین دو روز با بررسی تداخل، الگو و تأیید مسئول.","CAS Shift Swap",`<button class="btn btn-primary">درخواست جابه‌جایی</button>`)}
  ...
  function renderMyAttendance(){
    return `<div class="page">${sectionBack()}${pageHeader("حضور و کارکرد من","رخدادهای خام، تطبیق با برنامه شیفت، ورود/خروج مؤثر و وضعیت روزانه.","CAS Attendance Core",`<button class="btn">اعلام مغایرت</button>`)}
  ...
  function renderMyKardex(){
    return `<div class="page">${sectionBack()}${pageHeader("کاردکس من","محاسبه دقیقه‌ای بر پایه شیفت، حضور قطعی، درخواست‌های تأییدشده و اضافه‌کاری.","CAS Kardex Management",`<button class="btn">چاپ ماه</button><button class="btn btn-primary" data-route="attendance-requests">ثبت درخواست</button>`)}
  ...
  function renderAttendanceRequests(){
    return `<div class="page">${sectionBack()}${pageHeader("مرخصی و مأموریت","درخواست ساعتی، روزانه یا چندروزه با کنترل تقویم، شیفت و مسیر تأیید.","CAS Attendance Request",`<button class="btn btn-primary" data-action="new-request">درخواست جدید</button>`)}
  ...
  function renderOvertime(){
    return `<div class="page">${sectionBack()}${pageHeader("اضافه‌کاری","درخواست تأیید اضافه‌کاری انجام‌شده و مجوزهای خودکار تاریخ‌دار.","CAS Overtime",`<button class="btn btn-primary">ثبت اضافه‌کاری</button>`)}
  ...
  function renderWorkReportNew(){
    const role = D.roles[currentRole];
    const profiles = {
      guard:{title:"گزارش شیفت نگهبانی",code:"WRP-SEC-01",scope:"انتظامات • پست درب شمالی",approval:"تأیید مسئول انتظامات",deadline:"تا ۲ ساعت پس از تحویل شیفت",stationLabel:"پست نگهبانی",station:"درب شمالی",sectionTitle:"رخدادها و اقدامات شیفت",activityHeaders:["زمان","نوع رخداد","شرح و اقدام انجام‌شده","وضعیت","شاهد"],rows:[["۰۸:۱۰","کنترل تردد","بررسی مجوز ورود خودروی پیمانکار","تکمیل","دفتر تردد"],["۱۱:۴۰","گشت ایمنی","بازدید محوطه شمالی و اعلام مانع مسیر","نیازمند پیگیری","تصویر"]]},
      secretariat:{title:"گزارش روزانه دبیرخانه",code:"WRP-ADM-SEC-02",scope:"دبیرخانه مرکزی",approval:"تأیید مدیر امور اداری",deadline:"تا پایان روز کاری",stationLabel:"محل خدمت",station:"دبیرخانه مرکزی",sectionTitle:"ثبت و پیگیری مکاتبات",activityHeaders:["زمان","نوع فعالیت","موضوع","خروجی","پیگیری"],rows:[["۰۸:۳۰","نامه وارده","درخواست خرید قطعات","ثبت و ارجاع شد","ندارد"],["۱۳:۲۰","نامه صادره","پاسخ استعلام تأمین‌کننده","پیش‌نویس آماده شد","امضای مدیر"]]},
      system_admin:{title:"گزارش خدمات و رخدادهای فناوری اطلاعات",code:"WRP-IT-01",scope:"فناوری اطلاعات",approval:"تأیید مدیر امور اداری",deadline:"تا ۱۲ ساعت پس از پایان کار",stationLabel:"سایت/سامانه",station:"زیرساخت مرکزی",sectionTitle:"خدمات، رخدادها و تغییرات",activityHeaders:["زمان","سامانه/تجهیز","نوع درخواست","اقدام","نتیجه"],rows:[["۰۹:۰۰","Odoo","رفع خطای دسترسی","بررسی ACL و اصلاح نقش","رفع شد"],["۱۴:۱۵","شبکه داخلی","افت ارتباط","بررسی سوئیچ و ثبت رخداد","پایش ادامه دارد"]]},
      manager:{title:"گزارش روزانه مدیریت واحد",code:"WRP-MGT-01",scope:"مدیریت تولید سایت",approval:"ثبت نهایی مدیریتی",deadline:"تا ساعت ۱۰ روز بعد",stationLabel:"حوزه گزارش",station:"مدیریت تولید سایت",sectionTitle:"تصمیم‌ها، پیگیری‌ها و موانع کلیدی",activityHeaders:["موضوع","اقدام مدیریتی","مسئول پیگیری","موعد","وضعیت"],rows:[["توقف خط ۲","هماهنگی تعمیرات و برنامه جبرانی","سرپرست تعمیرات","امروز ۱۸:۰۰","در جریان"],["کمبود مواد","تأیید درخواست خرید اضطراری","مدیر خرید","فردا","ارجاع شد"]]},
      ceo:{title:"گزارش روزانه مدیریت عالی",code:"WRP-EXE-01",scope:"مدیریت عالی",approval:"ثبت نهایی",deadline:"تا پایان روز بعد",stationLabel:"حوزه گزارش",station:"کل سازمان",sectionTitle:"تصمیم‌ها و موضوعات راهبردی",activityHeaders:["موضوع","تصمیم/جهت‌گیری","مالک اقدام","موعد","سطح اهمیت"],rows:[["ظرفیت تولید","بررسی سناریوی افزایش شیفت","معاونت اجرایی","هفته جاری","بالا"],["ریسک تأمین","مذاکره با تأمین‌کننده جایگزین","مدیر خرید","۳ روز","بحرانی"]]},
      default:{title:"گزارش روزانه عملیات",code:"WRP-OPS-01",scope:role.unit,approval:"تأیید سرپرست عملیات",deadline:"تا ۱۲ ساعت پس از پایان شیفت",stationLabel:"ایستگاه کاری",station:"جریان عملیات",sectionTitle:"فعالیت‌های انجام‌شده",activityHeaders:["زمان","نوع فعالیت","موضوع/خروجی","مدت","شاهد"],rows:[["۰۸:۰۰","عملیات جاری","کنترل شروع شیفت و آماده‌سازی ایستگاه","۴۵ دقیقه","—"],["۱۰:۳۰","پیگیری","رفع توقف کوتاه و ثبت علت","۳۰ دقیقه","گزارش توقف"]]}
    };
    const p = profiles[currentRole] || profiles.default;
    const headerCells = p.activityHeaders.map(x=>`<th>${x}</th>`).join("");
    const bodyRows = p.rows.map(r=>`<tr>${r.map((x,i)=>`<td>${i===0?`<input value="${x}" />`:`<input value="${x}" />`}</td>`).join("")}<td><button class="btn btn-sm">حذف</button></td></tr>`).join("");
    return `<div class="page">${pageHeader(p.title,`این صفحه از پروفایل گزارش کار «${p.code}» و نسخه منتشرشده فرم پویا بارگذاری شده است.`,`CAS Work Report + Form Engine`,`<button class="btn">ذخیره پیش‌نویس</button><button class="btn btn-primary" data-action="submit-work-report">ارسال برای تأیید</button>`)}
      <div class="report-profile-banner"><div><span class="eyebrow">فرم فعال برای این انتساب</span><strong>${p.title}</strong><small>${p.scope} • نسخه فرم ۳ • معتبر از ۱۴۰۵/۰۴/۰۱</small></div><div class="report-profile-tags">${badge(p.code,"blue")}${badge("پروفایل پویا","green")}</div></div>
      <div class="form-shell"><section class="card form-card">
        <div class="form-section"><div class="section-head"><div><h3>اطلاعات خودکار گزارش</h3><p>اطلاعات هویتی، شیفت و بازه حضور از منابع سازمانی خوانده می‌شوند.</p></div><button class="btn btn-sm">مشاهده منبع اطلاعات</button></div><div class="form-grid">
          <div class="field"><label>کارمند</label><input value="${role.name}" readonly /></div><div class="field"><label>واحد و نقش</label><input value="${role.unit} • ${role.label}" readonly /></div>
          <div class="field"><label class="required">تاریخ گزارش</label><input value="۱۴۰۵/۰۴/۲۷" /></div><div class="field"><label>${p.stationLabel}</label><input value="${p.station}" readonly /></div>
          <div class="field"><label>شیفت/بازه مرتبط</label><input value="شیفت روز • ۰۷:۳۰ تا ۱۹:۳۰" readonly /></div><div class="field"><label>حضور ثبت‌شده</label><input value="۰۷:۲۸ تا ۱۹:۳۴" readonly /></div>
        </div></div>
        <div class="form-section"><div class="section-head"><div><h3>${p.sectionTitle}</h3><p>ساختار این بخش از نسخه فرم اختصاص‌یافته به شغل و واحد جاری تولید شده است.</p></div><div class="section-actions"><button class="btn btn-sm" data-action="add-standard-report-row">+ فعالیت از فهرست</button><button class="btn btn-primary btn-sm" data-action="open-new-activity-proposal">فعالیت در فهرست نیست</button></div></div>
          <div class="table-wrap"><table class="data-table report-activity-table"><thead><tr>${headerCells}<th></th></tr></thead><tbody>${bodyRows}</tbody></table></div>
          <div class="unlisted-activity-panel" style="margin-top:14px"><div class="unlisted-activity-head"><div><strong>فعالیت‌های ثبت‌شده خارج از فهرست</strong><small>این فعالیت‌ها مانع ارسال گزارش نمی‌شوند و عنوان اولیه کاربر در سابقه نگهداری می‌شود.</small></div>${badge(`${workActivities.filter(x=>x.standard===false).length} عنوان پیشنهادی`,workActivities.some(x=>x.standard===false)?"orange":"gray")}</div><div class="unlisted-activity-list">${workActivities.filter(x=>x.standard===false).length?workActivities.filter(x=>x.standard===false).map((item,i)=>`<article class="unlisted-activity-item"><div><strong>${esc(item.title)}</strong><small>${esc(item.category)} • ${toFaMinutes(item.duration)} • ${esc(item.note||'بدون توضیح')}</small></div><span class="inline-proposal-badge">عنوان پیشنهادی</span><button class="mini-btn" data-action="remove-proposed-report-activity" data-proposed-index="${i}" title="حذف از گزارش">×</button></article>`).join(''):`<div class="empty-inline-state"><strong>فعالیت خارج از فهرست ثبت نشده است.</strong><small>در صورت نبود عنوان مناسب، از دکمه «فعالیت در فهرست نیست» استفاده کنید.</small></div>`}</div></div>
          <div class="field" style="margin-top:14px"><label>جمع‌بندی آزاد روز</label><textarea placeholder="توضیح تکمیلی، نتیجه کلی یا نکته‌ای که در ردیف‌های ساختاریافته پوشش داده نشده است..."></textarea></div>
        </div>
        <div class="form-section"><h3>موانع، پیگیری و شواهد</h3><div class="form-grid"><div class="field"><label>موانع و مسائل</label><textarea placeholder="موضوعات نیازمند پیگیری سرپرست یا واحد دیگر..."></textarea></div><div class="field"><label>اقدام یا پیگیری بعدی</label><textarea placeholder="مسئول پیگیری، موعد و اقدام بعدی..."></textarea></div></div><div class="upload-box" style="margin-top:14px"><strong>فایل، تصویر یا کد شاهد را اضافه کنید</strong>پیوست‌ها در هسته اسناد نگهداری و به Submission همین نسخه فرم پیوند می‌شوند.</div></div>
      </section><aside class="form-aside"><section class="card"><div class="card-header"><h3 class="card-title">کنترل گزارش</h3></div><div class="card-body"><div class="status-panel" style="grid-template-columns:1fr"><div class="status-item"><span class="status-light"></span><div><strong>پروفایل معتبر</strong><small>${p.code} برای انتساب جاری Resolve شد</small></div></div><div class="status-item"><span class="status-light"></span><div><strong>اطلاعات زمینه‌ای</strong><small>کارمند، واحد و شیفت از منابع سازمانی</small></div></div><div class="status-item"><span class="status-light warn"></span><div><strong>تکمیل فرم</strong><small>۲ بخش هنوز نیازمند تکمیل است</small></div></div></div><div class="kv" style="margin-top:14px"><span>مهلت ارسال</span><strong>${p.deadline}</strong></div><div class="kv"><span>مسیر پس از ارسال</span><strong>${p.approval}</strong></div><div class="kv"><span>نسخه فرم</span><strong>نسخه ۳ • Snapshot محفوظ</strong></div></div></section>
        <section class="card" style="margin-top:14px"><div class="card-header"><h3 class="card-title">چرا این فرم نمایش داده شد؟</h3></div><div class="card-body"><p class="helper-text">سیستم براساس شرکت، انتساب مؤثر، واحد، شغل، شیفت و تاریخ اعتبار، پروفایل مناسب را انتخاب کرده است. تغییر فرم فقط از تنظیمات پروفایل گزارش کار انجام می‌شود.</p></div></section>
      </aside></div>
    </div>`;
  }
  ...
  function renderAttendanceHub(){
    return `<div class="page">${pageHeader("حضور، شیفت و کارکرد","شیفت امروز، ثبت‌های ورود و خروج، کارکرد ماه و درخواست‌های اصلاح در یک صفحه.","فضای شخصی",`<button class="btn" data-route="shift-swaps">جابه‌جایی شیفت</button><button class="btn btn-primary" data-route="attendance-requests">ثبت درخواست</button>`)}
      <div class="grid grid-4">${stat("◷","روز ۰۷:۳۰–۱۹:۳۰","شیفت امروز")}${stat("⇩","۰۷:۵۸","ورود ثبت‌شده")}${stat("▦","۱۷۶:۳۰","کارکرد ماه")}${stat("!","۱","ثبت ناقص تردد","خروج ثبت نشده","warn")}</div>

      <div class="grid grid-3" style="margin-top:18px">
        <section class="card span-2">
          <div class="card-header"><div><h3 class="card-title">وضعیت امروز</h3><div class="card-subtitle">تطبیق برنامه شیفت و رخدادهای ثبت‌شده</div></div>${badge("نیازمند تکمیل","orange")}</div>
          <div class="card-body"><div class="timeline">
            <div class="timeline-item"><h5>شروع شیفت برنامه‌ریزی‌شده</h5><p>شیفت روز</p><time>۰۷:۳۰</time></div>
            <div class="timeline-item"><h5>ورود از دستگاه درب اصلی</h5><p>ثبت قطعی</p><time>۰۷:۵۸</time></div>
            <div class="timeline-item"><h5>خروج ثبت نشده است</h5><p>پس از پایان شیفت می‌توانید درخواست ثبت خروج ایجاد کنید.</p><time>۱۹:۳۰</time></div>
          </div></div>
        </section>
        ${card("عملیات سریع",`<div class="grid"><button class="btn" data-route="my-attendance">حضور و کارکرد من</button><button class="btn" data-route="my-shifts">تقویم شیفت من</button><button class="btn" data-route="shift-swaps">جابه‌جایی شیفت</button><button class="btn" data-route="my-kardex">کاردکس من</button></div>`)}
      </div>

      <div class="grid grid-2" style="margin-top:18px">
        <section class="card" style="border-color:#f2cf8f">
          <div class="card-header"><div><h3 class="card-title">ثبت ناقص تردد</h3><div class="card-subtitle">برای کاربر فقط عدم ثبت ورود یا عدم ثبت خروج نمایش داده می‌شود.</div></div>${badge("۱ مورد","orange")}</div>
          <div class="card-body">
            <div class="record-row active" style="border:1px solid var(--ink-100);border-radius:14px;padding:14px">
              <div class="task-source">⇧</div><div><h4>خروج روز ۲۷ تیر ثبت نشده</h4><p>شیفت برنامه‌ریزی‌شده: ۰۷:۳۰ تا ۱۹:۳۰</p>${badge("نیازمند درخواست","orange")}</div><button class="btn btn-primary" data-route="attendance-requests">درخواست ثبت خروج</button>
            </div>
            <div class="proposal-note" style="margin-top:14px"><strong>مسیر درخواست</strong><p>ثبت ساعت پیشنهادی توسط شما ← تأیید سرپرست مستقیم ← ثبت رکورد اصلاحی در کاردکس. رکورد خام دستگاه تغییر نمی‌کند.</p></div>
          </div>
        </section>

        <section class="card">
          <div class="card-header"><div><h3 class="card-title">تأخیر نیازمند اقدام</h3><div class="card-subtitle">تأخیر از ثبت ناقص ورود و خروج جدا نگه داشته می‌شود.</div></div>${badge("۱ مورد","blue")}</div>
          <div class="card-body">
            <div class="record-row" style="border:1px solid var(--ink-100);border-radius:14px;padding:14px">
              <div class="task-source">◷</div><div><h4>۲۸ دقیقه تأخیر در ورود</h4><p>۲۷ تیر • ورود ۰۷:۵۸ به‌جای ۰۷:۳۰</p>${badge("در انتظار اقدام شما","blue")}</div>
            </div>
            <div class="grid grid-3" style="margin-top:14px"><button class="btn" data-route="attendance-requests">درخواست مأموریت</button><button class="btn" data-route="attendance-requests">درخواست مرخصی</button><button class="btn" data-route="attendance-requests">ثبت توضیح</button></div>
            <p class="card-subtitle" style="margin-top:12px;line-height:1.9">پس از تأیید درخواست، کاردکس دوباره محاسبه می‌شود؛ تأخیر پیش از تأیید حذف نخواهد شد.</p>
          </div>
        </section>
      </div>

      <section class="card table-card" style="margin-top:18px">
        <div class="card-header"><div><h3 class="card-title">درخواست‌های اصلاح اخیر</h3><div class="card-subtitle">وضعیت درخواست، تأیید سرپرست و ثبت نهایی در کاردکس</div></div><button class="btn btn-sm" data-route="request-tracking">مشاهده همه</button></div>
        <div class="table-wrap"><table class="data-table"><thead><tr><th>درخواست</th><th>تاریخ</th><th>ساعت پیشنهادی</th><th>مرحله</th><th>وضعیت کاردکس</th><th></th></tr></thead><tbody>
          <tr><td class="title-cell"><strong>ثبت ورود</strong><small>درخواست اصلاح تردد</small></td><td>۲۲ تیر ۱۴۰۵</td><td>۰۷:۳۲</td><td>${badge("تأیید سرپرست","green")}</td><td>${badge("رکورد اصلاحی ثبت شد","green")}</td><td><button class="btn btn-sm" data-route="my-kardex">مشاهده</button></td></tr>
          <tr><td class="title-cell"><strong>ثبت خروج</strong><small>درخواست اصلاح تردد</small></td><td>۱۸ تیر ۱۴۰۵</td><td>۱۹:۳۶</td><td>${badge("ردشده","red")}</td><td>${badge("بدون تغییر","gray")}</td><td><button class="btn btn-sm" data-route="request-tracking">جزئیات</button></td></tr>
        </tbody></table></div>
      </section>
  ...
  function chatMessages(name){
    const samples=[
      ...
      ["مریم فلاحی","مقدار سفارش در نسخه جدید اصلاح شد. لطفاً ردیف دوم جدول را هم کنترل کنید.","۰۹:۳۸","other"],
      ["مهدی رضایی","ردیف دوم با موجودی انبار تطبیق داده شد؛ اختلافی مشاهده نشد.","۰۹:۴۴","me"],
      ["مریم فلاحی","زمان تحویل پیشنهادی تأمین‌کننده سه‌شنبه آینده است. با برنامه تولید سازگار است؟","۰۹:۵۲","other"],
      ["مهدی رضایی","بله، با برنامه فعلی سازگار است؛ فقط باید تأیید نهایی سرپرست تولید ثبت شود.","۱۰:۰۱","me"],
      ["مریم فلاحی","درخواست تأیید برای سرپرست تولید ارسال شد و در کارتابل ایشان قرار گرفت.","۱۰:۱۲","other"],
      ["مهدی رضایی","سپاس. وضعیت اقدام را تا دریافت پاسخ باز نگه می‌دارم.","۱۰:۱۹","me"],
      ["مریم فلاحی","سرپرست تولید تأیید کرد؛ توضیح کوتاهی هم درباره اولویت تحویل ثبت کرده است.","۱۰:۳۴","other"],
      ["مهدی رضایی","توضیح را دیدم. نسخه نهایی نامه خرید را آماده می‌کنم.","۱۰:۴۱","me"],
      ["مریم فلاحی","شماره پیگیری تأمین‌کننده ۸۴۲۱ است؛ برای مراجعات بعدی در نامه درج شود.","۱۰:۵۰","other"],
      ["مهدی رضایی","شماره پیگیری اضافه شد و کنترل نهایی پیوست‌ها هم انجام شد.","۱۱:۰۲","me"],
      ["مریم فلاحی","همه موارد کامل است. نامه برای امضای مدیر ارسال شد.","۱۱:۱۴","other"],
      ["مهدی رضایی","عالی است. بعد از امضا، نسخه نهایی را در همین گفتگو ثبت می‌کنم.","۱۱:۲۰","me"]
    ];
  ...
  function scrollActiveChatToBottom(){
    if(currentRoute!=="messages") return;
  ...
  function renderRoute(){
    ...
    app.innerHTML = fn();
    const dashboardSettingsTopbar=document.getElementById("dashboardSettingsTopbar");
    if(dashboardSettingsTopbar) dashboardSettingsTopbar.hidden=currentRoute!=="home";
  ...
  function globalSearch(){
    const recent = [
      {icon:"✉",title:"درخواست تأمین قطعات یدکی خط شات‌بلاست",meta:"مکاتبات • امروز ۰۹:۲۱",route:"correspondence"},
      {icon:"▤",title:"فرم درخواست مرخصی ساعتی",meta:"فرم‌ها • دیروز",route:"form-catalog"},
      {icon:"♙",title:"مریم فلاحی",meta:"کارمند • مدیریت اداری",route:"home"},
      {icon:"▧",title:"قرارداد سرویس دستگاه حضور و غیاب",meta:"اسناد • ۲۵ تیر",route:"documents"}
    ];
    openModal("جست‌وجوی سازمان",`<div class="org-search-modal">
      <div class="org-search-box"><span>⌕</span><input id="orgSearchInput" autofocus autocomplete="off" placeholder="نام شخص، فرم، نامه، سند یا اقدام..." /><kbd>Esc</kbd></div>
      <div class="org-search-filters" id="orgSearchFilters"><button class="active" data-search-type="all">همه</button><button data-search-type="people">اشخاص</button><button data-search-type="actions">اقدام‌ها</button><button data-search-type="letters">نامه‌ها</button><button data-search-type="documents">اسناد</button><button data-search-type="forms">فرم‌ها</button></div>
      <div id="orgSearchResults" class="org-search-results" data-active-type="all">
        <div class="org-search-section-head"><strong>اخیراً مشاهده‌شده</strong><small>برای بازگشت سریع؛ نه گزارش ممیزی</small></div>
        ${recent.map(x=>`<button class="org-search-result" data-route="${x.route}"><span class="result-icon">${x.icon}</span><span><strong>${x.title}</strong><small>${x.meta}</small></span><b>‹</b></button>`).join("")}
      </div>
      <div class="org-search-hint"><span>↑ ↓ پیمایش</span><span>Enter بازکردن</span><span>Ctrl K فراخوانی سریع</span></div>
    </div>`);
    setTimeout(()=>document.getElementById("orgSearchInput")?.focus(),0);
  }

  function renderOrganizationSearchResults(){
    const input=document.getElementById("orgSearchInput"), host=document.getElementById("orgSearchResults");
    if(!input||!host)return;
    const q=input.value.trim().toLowerCase();
    const type=host.dataset.activeType||"all";
    if(!q){
      const recent=[
        {icon:"✉",title:"درخواست تأمین قطعات یدکی خط شات‌بلاست",meta:"مکاتبات • امروز ۰۹:۲۱",route:"correspondence"},
        {icon:"▤",title:"فرم درخواست مرخصی ساعتی",meta:"فرم‌ها • دیروز",route:"form-catalog"},
      ...
      host.innerHTML=`<div class="org-search-section-head"><strong>اخیراً مشاهده‌شده</strong><small>برای بازگشت سریع؛ نه گزارش ممیزی</small></div>${recent.map(x=>`<button class="org-search-result" data-route="${x.route}"><span class="result-icon">${x.icon}</span><span><strong>${x.title}</strong><small>${x.meta}</small></span><b>‹</b></button>`).join("")}`;
      return;
    }
    const groups=[];
    const add=(key,label,items)=>{ if((type==="all"||type===key)&&items.length) groups.push({label,items}); };
    add("people","اشخاص",D.people.filter(x=>(x.name+" "+x.job+" "+x.unit).toLowerCase().includes(q)).slice(0,6).map(x=>({icon:x.initials,title:x.name,meta:`${x.job} • ${x.unit}`,route:"home"})));
    add("actions","اقدام‌ها",D.actions.filter(x=>(x.title+" "+x.subject+" "+x.source).toLowerCase().includes(q)).slice(0,6).map((x,i)=>({icon:x.icon,title:x.title,meta:`${x.source} • ${x.subject}`,action:"open-action",index:i})));
    add("letters","نامه‌ها",D.letters.filter(x=>(x.subject+" "+x.no+" "+x.from).toLowerCase().includes(q)).slice(0,6).map(x=>({icon:"✉",title:x.subject,meta:`${x.no} • ${x.from}`,route:"correspondence"})));
    add("documents","اسناد",D.documents.filter(x=>(x.name+" "+x.folder+" "+x.format).toLowerCase().includes(q)).slice(0,6).map(x=>({icon:"▧",title:x.name,meta:`${x.folder} • ${x.format}`,route:"documents"})));
    add("forms","فرم‌ها",D.forms.filter(x=>(x.title+" "+x.code+" "+x.category).toLowerCase().includes(q)).slice(0,6).map(x=>({icon:"▤",title:x.title,meta:`${x.code} • ${x.category}`,route:"form-catalog"})));
    host.innerHTML=groups.length?groups.map(g=>`<section><div class="org-search-section-head"><strong>${g.label}</strong><small>${num(g.items.length)} نتیجه</small></div>${g.items.map(x=>`<button class="org-search-result" ${x.route?`data-route="${x.route}"`:`data-action="${x.action}" data-index="${x.index}"`}><span class="result-icon">${x.icon}</span><span><strong>${x.title}</strong><small>${x.meta}</small></span><b>‹</b></button>`).join("")}</section>`).join(""):`<div class="org-search-empty"><strong>نتیجه‌ای پیدا نشد</strong><small>عبارت یا نوع نتیجه را تغییر دهید.</small></div>`;
  }
  ...
  function handleAction(action,target){
    switch(action){
      ...
      case "open-dashboard-settings": dashboardSettingsDrawer(); break;
      case "toggle-dashboard-widget": {
        const id=target.dataset.widget; const hidden=new Set(v7Settings.hiddenWidgets||[]); hidden.has(id)?hidden.delete(id):hidden.add(id); v7Settings.hiddenWidgets=[...hidden]; applyV7Settings(); dashboardSettingsDrawer(); break;
      }
      case "toggle-dashboard-shortcut": {
        const id=target.dataset.shortcut; const list=[...(v7Settings.shortcuts||[])]; const i=list.indexOf(id); if(i>=0)list.splice(i,1); else list.push(id); v7Settings.shortcuts=list.slice(0,10); applyV7Settings(); dashboardSettingsDrawer(); break;
      }
      case "move-dashboard-shortcut-up": {
        const id=target.dataset.shortcut; const list=[...(v7Settings.shortcuts||[])]; const i=list.indexOf(id); if(i>0){[list[i-1],list[i]]=[list[i],list[i-1]];v7Settings.shortcuts=list;applyV7Settings();} dashboardSettingsDrawer(); break;
      }
      case "move-dashboard-shortcut-down": {
        const id=target.dataset.shortcut; const list=[...(v7Settings.shortcuts||[])]; const i=list.indexOf(id); if(i>=0&&i<list.length-1){[list[i+1],list[i]]=[list[i],list[i+1]];v7Settings.shortcuts=list;applyV7Settings();} dashboardSettingsDrawer(); break;
      }
      case "save-dashboard-settings": closeLayers(); applyV7Settings(); renderRoute(); toast("تنظیمات میزکار ذخیره و اعمال شد."); break;
      case "reset-dashboard-settings": v7Settings.hiddenWidgets=[]; v7Settings.shortcuts=[...v7Defaults.shortcuts]; v7Settings.shortcutSuggestions=true; v7Settings.widgetSettings=JSON.parse(JSON.stringify(v7Defaults.widgetSettings)); applyV7Settings(); dashboardSettingsDrawer(); toast("تنظیمات میزکار بازنشانی شد."); break;
      case "set-v7-setting":
      ...
      case "action-source-filter":
        actionSourceFilter=target.dataset.value||"all"; renderRoute(); break;
      case "action-time-filter":
        actionTimeFilter=target.dataset.value||"all"; renderRoute(); break;
      ...
      case "open-new-activity-proposal": newActivityProposalModal(); break;
      case "add-standard-report-row": toast("انتخاب فعالیت استاندارد از فهرست پروفایل گزارش باز می‌شود."); break;
      case "remove-proposed-report-activity": {
        const proposed=workActivities.map((x,i)=>({x,i})).filter(v=>v.x.standard===false);
        const actual=proposed[Number(target.dataset.proposedIndex||0)]?.i;
        if(actual!==undefined){workActivities.splice(actual,1);toast("فعالیت پیشنهادی از گزارش حذف شد.","warn");renderRoute();}
        break;
      }
      case "toggle-activity-combobox": {
        const panel=document.getElementById("quickActivityPanel");
        const trigger=target.closest(".activity-combobox-trigger");
        if(!panel) break;
        const opening=panel.hidden;
        panel.hidden=!opening;
        trigger?.setAttribute("aria-expanded",String(opening));
        if(opening){setTimeout(()=>document.getElementById("quickActivitySearch")?.focus(),0);}
        break;
      }
      case "select-quick-activity": {
        const value=target.dataset.value||"";
        const input=document.getElementById("quickActivity");
        const label=document.getElementById("quickActivityLabel");
        const panel=document.getElementById("quickActivityPanel");
        if(value==="فعالیت در فهرست نیست"){
          if(input)input.value="";
          if(label)label.textContent="انتخاب یا جست‌وجوی فعالیت";
          if(panel)panel.hidden=true;
          newActivityProposalModal();
          break;
        }
        if(input)input.value=value;
        if(label)label.textContent=value;
        if(panel)panel.hidden=true;
        document.querySelector('[data-action="toggle-activity-combobox"]')?.setAttribute("aria-expanded","false");
        break;
      }
      case "add-quick-activity": {
        const selected=document.getElementById("quickActivity")?.value.trim()||"";
        if(selected==="فعالیت در فهرست نیست"){newActivityProposalModal();break;}
        const activityCatalog={"بررسی و پاسخ به مکاتبات عملیات":"مکاتبات","تحلیل مغایرت حضور شیفت شب":"حضور و کاردکس","پیگیری اقدام‌های باز":"اقدام‌ها","جلسه هماهنگی عملیات":"جلسه"};
        const title=selected;
        const category=activityCatalog[selected];
        if(!title || !category){toast("یک فعالیت معتبر را از فهرست جست‌وجو و انتخاب کنید.","warn");break;}
        const selectedDuration=document.getElementById("quickDuration")?.value||"30";
        const duration=selectedDuration==="custom"?Number(document.getElementById("quickDurationCustom")?.value||0):Number(selectedDuration);
        if(!duration || duration<1){toast("زمان دلخواه را به دقیقه وارد کنید.","warn");break;}
      ...
      case "confirm-new-activity": {
        ...
        const selectedDuration=document.getElementById("proposalDuration")?.value||"30";
        const duration=selectedDuration==="custom"?Number(document.getElementById("proposalDurationCustom")?.value||0):Number(selectedDuration);
        if(!duration || duration<1){toast("زمان دلخواه را به دقیقه وارد کنید.","warn");break;}
      ...
      case "customize-workspace": dashboardSettingsDrawer(); break;
  ...
  document.addEventListener("change",e=>{
    const setting=e.target.closest("[data-widget-setting]");
    if(setting){const [widget,key]=setting.dataset.widgetSetting.split("."); const value=setting.type==="checkbox"?setting.checked:setting.value; v7Settings.widgetSettings[widget][key]=value; if(widget==="calendar"&&key==="defaultView"){homeCalendarView=value;localStorage.setItem("cas.home.calendarView",value);} applyV7Settings(); return;}
    if(e.target.id==="shortcutSuggestions"){v7Settings.shortcutSuggestions=e.target.checked;applyV7Settings();}
  });
  ...
  document.addEventListener("click",e=>{
    ...
    const searchType=e.target.closest("[data-search-type]");
    if(searchType){ document.querySelectorAll("[data-search-type]").forEach(x=>x.classList.toggle("active",x===searchType)); const host=document.getElementById("orgSearchResults"); if(host){host.dataset.activeType=searchType.dataset.searchType; renderOrganizationSearchResults();} return; }
    ...
    if(navToggle){
      ...
      saved[id]=willOpen;
      localStorage.setItem("cas.nav.open",JSON.stringify(saved));
      tree.classList.toggle("open",willOpen);
      navToggle.setAttribute("aria-expanded",String(willOpen));
      navToggle.setAttribute("aria-label",`${willOpen?"بستن":"بازکردن"} زیرمنو`);
      navToggle.title=`${willOpen?"بستن":"بازکردن"} زیرمنو`;
      const children=tree.querySelector(".nav-children");
      if(children) children.setAttribute("aria-hidden",String(!willOpen));
      return;
    }
  ...
  document.addEventListener("input",event=>{
    if(event.target.id==="quickActivitySearch"){
      const q=event.target.value.trim().toLowerCase();
      document.querySelectorAll("#quickActivityResults [data-value]").forEach(option=>{
        option.hidden=q && !option.dataset.value.toLowerCase().includes(q);
      });
    }
    if(["participantLookup","participantUnit","participantScope"].includes(event.target.id)) renderParticipantResults();
    if(event.target.id==="orgSearchInput") renderOrganizationSearchResults();
  });
  document.addEventListener("change",event=>{
    if(event.target.id==="quickDuration"){
      const custom=document.getElementById("quickDurationCustom");
      const host=event.target.closest(".duration-control");
      const show=event.target.value==="custom";
      if(custom){custom.hidden=!show;if(show)custom.focus();else custom.value="";}
      if(host)host.classList.toggle("is-custom",show);
      return;
    }
    if(event.target.id==="proposalDuration"){
      const custom=document.getElementById("proposalDurationCustom");
      const host=event.target.closest(".duration-control");
      const show=event.target.value==="custom";
      if(custom){custom.hidden=!show;if(show)custom.focus();else custom.value="";}
      if(host)host.classList.toggle("is-custom",show);
