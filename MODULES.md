...
این مخزن شامل ۲۴ ماژول است. مستندات هر ماژول سه لایه دارد:

1. `README.md`: معرفی، اجزای اصلی، نقش‌ها و شروع سریع.
2. `docs/ARCHITECTURE_AND_USAGE.md`: معماری، مرز دامنه، سناریوهای نقش‌محور، امنیت، آزمون و عیب‌یابی.
3. `docs/TECHNICAL_REFERENCE.md`: مرجع استخراج‌شده از کد شامل مدل‌ها، تمام فیلدها، متدها، constraintها، ACL، record rule، منو، action، cron، sequence، asset و تست.
...
هر تغییر در مدل، نقش، منو، چرخه وضعیت، API، asset یا وابستگی باید در همان commit در هر سه لایه مرتبط منعکس شود. مستند ناسازگار با کد یک نقص انتشار محسوب می‌شود.
